...
```{r exercise 01-2}
diamonds[colnames(diamonds) %in% c('price', 'cut',  'color')]
```
...
```{r exercise 08-1}
presidential %>% 
  sort_by(presidential$party) 
```

Arrange the table by start date in the descending order.
```{r exercise 08-2}
presidential %>% 
  sort_by(desc(presidential$start))
...
```{r exercise 10-1}
sleep_bodywt <- msleep %>% 
  mutate(sleep_bodywt = sleep_total/bodywt)
```

Compute new variable / column `is_big` of logical value that are `TRUE` if body weight is above `50`.
```{r exercise 10-2}
sleep_bodywt = sleep_bodywt %>% 
  mutate(is_big = if_else(bodywt > 50, TRUE, FALSE))
```
...
```{r exercise 10-3}
sleep_bodywt = sleep_bodywt %>% 
  mutate(mean_sleep = mean(sleep_total), sd_sleep = sd(sleep_total), z_sleep = (sleep_total - mean(sleep_total)) / sd(sleep_total))
```

Compute z-score for body weight variable, do not store mean and standard deviation in extra columns but use them directly.
```{r exercise 10-4}
sleep_bodywt = sleep_bodywt %>% 
  mutate(z_bwt = (bodywt - mean(bodywt)) / sd(bodywt))
